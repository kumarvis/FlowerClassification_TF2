...
class LRFinder(object):
    ...
    def range_test(self, min_lr, max_lr, num_itrs=100, debug=True):
        ...
        for step, (x, y) in enumerate(self.dataset):
            ...
            if step_plus > 1 and smoothed_loss > 4 * best_loss:
    ...
    def dump(self):
        if os.path.exists("optimal_lr.log") == True:
            os.remove("optimal_lr.log")
        logging.basicConfig(filename='optimal_lr.log', filemode='w', format='%(message)s',
                            level=logging.INFO)
    ...
    def plot_smooth_curve(self, log_file_name='optimal_lr.log'):
